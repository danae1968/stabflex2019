...
% here add directory that files were saved
...
todo.Colortest=1; % color sensitivity test
...
if todo.Colorwheel 
    ...
    [statmatAcc,statmatRT,statmatSZ]=performanceCW(io.saveD,io.subNo,trialData,io);
...
if todo.ChoiceNR 
           fprintf(1,[repmat('-',1,72),'\n'])
        fprintf(1,'Running ''Analyzing Indifference Points for No Redo choices'' \n')
        
    io.resultsDir=fullfile(io.projectDir,'results','Choices',sprintf('N%d',max(io.subNo)));
    if ~exist(io.resultsDir,'dir')
        mkdir(io.resultsDir)
    end
    
    io.dataDir=fullfile(io.projectDir,'data','ChoiceTask');
    [IPmatrixNR]=regressionChoicesNoRedo(io.saveD,io.doPlots,io.subNo,io);
    [choicesNR]=choicesR(io.saveD,io.subNo,io,1);
    [PerRedo,PerSZ]=PercentageEasy(io.saveD,io.subNo,choicesNR,io,1);
    end
...
if todo.ChoiceD
    ...
    io.dataDir=fullfile(io.projectDir,'data','ChoiceTask');
    [IPmatrixD]=DirectComparisonAnalysis(io.saveD,io.subNo,io,io.doPlots);
    [choicesDir]=choicesR(io.saveD,io.subNo,io,2);
    [perIgnore]=PercentageEasy(io.saveD,io.subNo,choicesDir,io,2);
